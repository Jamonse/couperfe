...
import { COMPANY_DELETE_URL, COMPANY_GET_ALL_URL, COMPANY_GET_EXAMPLE_URL, COMPANY_GET_PAGED_URL, COMPANY_GET_URL, COMPANY_POST_URL, COMPANY_PUT_URL } from '../../utils/api.utils';
...
export class CompaniesService {
  ...
  getAllCompanies(pageIndex: number, pageSize: number, sortBy: string, asc: boolean): Observable<any>
  {
    if(arguments.length == 0)
    { // No paing arguments - get all companies
      return this.httpClient.get<Company[]>(COMPANY_GET_ALL_URL);
    }
    else
    { // Get comapnies based on the paging arguments
